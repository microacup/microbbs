(function(_global){
  function EditorExt(options) {
    options = options || {};
    this.upload = options.upload || '/upload';
    ...
    //图片上传工具
    var ToolImage = function (_upload) {
      var self = this;
      this.$win = $([
        '<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="editorToolImageTitle" aria-hidden="true" style="display: none;">' +
        '<div class="modal-dialog" role="document">' +
      ...
      this.uploader.on('uploadSuccess', function (file, res) {
        if (res.code == 200) {
          $(".close").click();
          $("#error_message").html("");
          var image = '![' + file.name + '](' + _upload + '/' + res.data + ')';
    ...
    var toolImage = new ToolImage(this.upload);
